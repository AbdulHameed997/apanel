...
(function (factory) {
  ...
  function updateSelectionStates(dualListbox) {
    dualListbox.element.find('option')
      ...
      .each(function (index, item) {
        ...
        if ($item.prop('selected')) {
          ...
          dualListbox.elements.select2.append($item.clone(true)
            .prop('selected', $item.data('_selected')))
        } else {
          dualListbox.elements.select1.append($item.clone(true)
            .prop('selected', $item.data('_selected')))
        }
      })
  ...
  function filter(dualListbox, selectIndex) {
    ...
    options.each(function (index, item) {
      var $item = $(item),
        isFiltered = true
      if (item.text.match(regex) || (dualListbox.settings.filterOnValues && $item.attr('value')
        .match(regex))) {
        isFiltered = false
        dualListbox.elements['select' + selectIndex].append($item.clone(true)
          .prop('selected', $item.data('_selected')))
      }
  ...
  function saveSelections(dualListbox, selectIndex) {
    ...
    dualListbox.elements['select' + selectIndex].find('option')
      .each(function (index, item) {
        var $item = $(item)
        options.eq($item.data('original-index'))
    ...
    select.find('option')
      .sort(function (a, b) {
        return ($(a)
          ...
          .data('original-index')) ? 1 : -1
      })
      .appendTo(select)
  }

  function clearSelections(dualListbox) {
    dualListbox.elements.select1.find('option')
      .each(function () {
        dualListbox.element.find('option')
  ...
  function move(dualListbox) {
    ...
    dualListbox.elements.select1.find('option:selected')
      .each(function (index, item) {
        var $item = $(item)
  ...
  function remove(dualListbox) {
    ...
    dualListbox.elements.select2.find('option:selected')
      .each(function (index, item) {
        var $item = $(item)
  ...
  BootstrapDualListbox.prototype = {
    ...
    setSelectedListLabel: function (value, refresh) {
      ...
      if (value) {
        this.elements.label2.show()
          .html(value)
      } else {
        this.elements.label2.hide()
          .html(value)
      }
      if (refresh) {
        refreshSelects(this)
    ...
    setNonSelectedListLabel: function (value, refresh) {
      ...
      if (value) {
        this.elements.label1.show()
          .html(value)
      } else {
        this.elements.label1.hide()
          .html(value)
      }
      if (refresh) {
        refreshSelects(this)
    ...
    setBtnClass: function (value, refresh) {
      ...
      this.elements.moveButton.attr('class', 'btn move')
        .addClass(value)
      this.elements.removeButton.attr('class', 'btn remove')
        .addClass(value)
      this.elements.moveAllButton.attr('class', 'btn moveall')
        .addClass(value)
      this.elements.removeAllButton.attr('class', 'btn removeall')
        .addClass(value)
      if (refresh) {
        refreshSelects(this)
      }
