...
(function (factory) {
  ...
  Mapael.prototype = {
    ...
    init: function () {
      ...
      self.$tooltip = $('<div>')
        .addClass(self.options.map.tooltip.cssClass)
        .css('display', 'none')
      ...
      $(self.paper.desc)
        .append(' and Mapael ' + self.version + ' (https://www.vincentbroute.fr/mapael/)')
    },
    ...
    destroy: function () {
      ...
      if (self.onResizeEvent) {
        $(window)
          .off('resize.' + pluginName, self.onResizeEvent)
      }
    ...
    initDelegatedCustomEvents: function () {
      ...
      $.each(self.customEventHandlers, function (eventName) {
        ...
        self.$container.off(fullEventName)
          .on(fullEventName, '[data-id]', function (e) {
            var $elem = $(this)
            var id = $elem.attr('data-id')
            var type = $elem.attr('data-type')
              .replace('-text', '')
    ...
    initZoom: function (mapWidth, mapHeight, zoomOptions) {
      ...
      $('body')
        .on('mouseup.' + pluginName + (zoomOptions.touch ? ' touchend.' + pluginName : ''), function () {
          mousedown = false
    ...
    onUpdateEvent: function (e, opt) {
      ...
      if (typeof opt.mapOptions === 'object') {
        if (opt.replaceOptions === true) {
          self.options = self.extendDefaultOptions(opt.mapOptions)
        } else {
          $.extend(true, self.options, opt.mapOptions)
        }

        // IF we update areas, plots or legend, then reset all legend state to "show"
        if (opt.mapOptions.areas !== undefined || opt.mapOptions.plots !== undefined || opt.mapOptions.legend !== undefined) {
          $('[data-type=\'legend-elem\']', self.$container)
      ...
      if (typeof opt.setLegendElemsState === 'object') {
        ...
        $.each(opt.setLegendElemsState, function (legendCSSClass, action) {
          ...
          if ($legend !== undefined) {
            ...
            $('[data-type=\'legend-elem\']', $legend)
              .each(function (id, elem) {
                if (($(elem)
                    .attr('data-hidden') === '0' && action === 'hide') ||
                  ($(elem)
                    .attr('data-hidden') === '1' && action === 'show')) {
                  // Toggle state of element by clicking
                  $(elem)
                    .trigger('click', {
                      hideOtherElems: false,
    ...
    drawLegend: function (legendOptions, legendType, elems, scale, legendIndex) {
      ...
      if (legendOptions.title && legendOptions.title !== '') {
        title = legendPaper.text(legendOptions.marginLeftTitle, 0, legendOptions.title)
      ...
      for (i = 0; i < sliceOptions.length; ++i) {
        ...
        if (sliceOptions[i].display === undefined || sliceOptions[i].display === true) {
          ...
          legendLabel = legendPaper.text(x, y, sliceOptions[i].label)
    ...
    handleClickOnLegendElem: function (elem, id, legendIndex, legendType, opts) {
      ...
      if ((opts.hideOtherElems === undefined || opts.hideOtherElems === true) && legendOptions.exclusive === true) {
        $('[data-type=\'legend-elem\'][data-hidden=0]', self.$container)
          .each(function () {
            var $elem = $(this)
    ...
    setHoverOptions: function (elem, originalAttrs, attrsHover) {
      ...
      if (elem.attrsHover.transform) {
        elem.originalAttrs = $.extend({ transform: 's1' }, originalAttrs)
      } else {
