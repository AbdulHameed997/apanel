...
(function (factory) {
  ...
  $.extend(RowGroup.prototype, {
    ...
    dataSrc: function (val) {
      ...
      $(dt.table()
        .node())
        .triggerHandler('rowgroup-datasrc.dt', [dt, val])
    ...
    _adjustColspan: function () {
      $('tr.' + this.c.className, this.s.dt.table()
        .body())
        .find('td')
    ...
    _draw: function () {
      var dt = this.s.dt
      var groupedRows = this._group(0, dt.rows({ page: 'current' })
        .indexes())
    ...
    _groupDisplay: function (level, groups) {
      ...
      for (var i = 0, ien = groups.length; i < ien; i++) {
        ...
        if (this.c.startRender) {
          ...
          if (row) {
            row.insertBefore(dt.row(rows[0])
              .node())
          }
        }
...
// events so we can automatically initialise
  $(document)
    .on('preInit.dt.dtrg', function (e, settings, json) {
      if (e.namespace !== 'dt') {
        return
      }
