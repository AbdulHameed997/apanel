...
(function (root, factory) {
  ...
  var DateRangePicker = function (element, options, cb) {
    ...
    if (typeof options.ranges === 'object') {
      for (range in options.ranges) {
        ...
        if (this.maxSpan && maxDate && start.clone()
          .add(this.maxSpan)
          .isAfter(maxDate)) {
          maxDate = start.clone()
            .add(this.maxSpan)
    ...
    if (this.singleDatePicker) {
      ...
      this.container.find('.drp-calendar.left')
        .addClass('single')
      this.container.find('.drp-calendar.left')
        .show()
      this.container.find('.drp-calendar.right')
        .hide()
      if (!this.timePicker) {
        this.container.addClass('auto-apply')
      }
  ...
  DateRangePicker.prototype = {
    ...
    setEndDate: function (endDate) {
      ...
      if (this.maxSpan && this.startDate.clone()
        .add(this.maxSpan)
      ...
      if (this.timePicker) {
        ...
        if (this.endDate) {
          ...
          minute = parseInt(this.container.find('.left .minuteselect')
            .val(), 10)
          if (isNaN(minute)) {
            minute = parseInt(this.container.find('.left .minuteselect option:last')
              .val(), 10)
          }
          second = this.timePickerSeconds ? parseInt(this.container.find('.left .secondselect')
            .val(), 10) : 0
          ...
          minute = parseInt(this.container.find('.right .minuteselect')
            .val(), 10)
          if (isNaN(minute)) {
            minute = parseInt(this.container.find('.right .minuteselect option:last')
              .val(), 10)
          }
    ...
    renderCalendar: function (side) {
      ...
      this.container.find('.drp-calendar.' + side + ' .calendar-table')
        .html(html)
    ...
    renderTimePicker: function (side) {
      ...
      if (this.maxSpan && (!this.maxDate || this.startDate.clone()
        .add(this.maxSpan)
      ...
      this.container.find('.drp-calendar.' + side + ' .calendar-time')
        .html(html)
    ...
    move: function () {
      ...
      if (this.opens == 'left') {
        ...
        if (containerWidth + containerRight > $(window)
          .width()) {
          this.container.css({
            top: containerTop,
            right: 'auto',
        ...
        if (containerLeft + containerWidth > $(window)
          .width()) {
          this.container.css({
            top: containerTop,
            left: 'auto',
    ...
    hoverDate: function (e) {
      ...
      if (!this.endDate) {
        this.container.find('.drp-calendar tbody td')
    ...
    timeChanged: function (e) {
      ...
      var second = this.timePickerSeconds ? parseInt(cal.find('.secondselect')
        .val(), 10) : 0
