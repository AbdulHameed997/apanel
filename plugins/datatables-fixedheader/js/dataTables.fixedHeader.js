...
(function (factory) {
  ...
  $.extend(FixedHeader.prototype, {
    ...
    _constructor: function () {
      ...
      $(window)
        ...
        .on('resize' + this.s.namespace, DataTable.util.throttle(function () {
          that.s.position.windowHeight = $(window)
            .height()
          that.update()
        }, 50))
    ...
    _clone: function (item, force) {
      ...
      if (!force && itemDom.floating) {
        ...
        if (itemDom.floating) {
          ...
          this._unsize(item)
          itemDom.floating.children()
            .detach()
          itemDom.floating.remove()
        }

        itemDom.floating = $(dt.table()
          .node()
          .cloneNode(false))
    ...
    _scroll: function (forceChange) {
      var windowTop = $(document)
        .scrollTop()
      var windowLeft = $(document)
        .scrollLeft()
      var position = this.s.position
      var headerMode,
        footerMode
...
// table and if so, initialise
  $(document)
    .on('init.dt.dtfh', function (e, settings, json) {
      if (e.namespace !== 'dt') {
        return
      }
