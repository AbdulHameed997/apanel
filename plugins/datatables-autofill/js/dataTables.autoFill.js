...
(function (factory) {
  ...
  $.extend(AutoFill.prototype, {
    ...
    _constructor: function () {
      var that = this
      var dt = this.s.dt
      var dtScroll = $('div.dataTables_scrollBody', this.s.dt.table()
        .container())
    ...
    _attach: function (node) {
      var dt = this.s.dt
      var idx = dt.cell(node)
        .index()
      var handle = this.dom.handle
      var handleDim = this.s.handle

      if (!idx || dt.columns(this.c.columns)
        .indexes()
        .indexOf(idx.column) === -1) {
        this._detach()
        return
      }
      ...
      if (!this.dom.offsetParent) {
        ...
        this.dom.offsetParent = $(dt.table()
          .node())
          .offsetParent()
      }
    ...
    _actionSelector: function (cells) {
      ...
      if (available.length === 1 && this.c.alwaysAsk === false) {
        ...
        var list = this.dom.list.children('ul')
          .empty()
    ...
    _drawSelection: function (target, e) {
      ...
      var endCell = $(dt.cell(endRow.index(), colIndx)
        .node())
    ...
    _focusListener: function () {
      ...
      if (focus === 'focus') {
        ...
        $(dt.table()
          .body())
          .on('click' + namespace, 'td, th', function (e) {
            that._attach(this)
          })
    ...
    _mousedown: function (e) {
      ...
      this.s.start = {
        row: dt.rows({ page: 'current' })
          .nodes()
          .indexOf($(this.dom.start)
            .parent()[0]),
        column: $(this.dom.start)
          .index()
      }
    ...
    _mouseup: function (e) {
      ...
      var enabledColumns = dt.columns(this.c.columns)
        .indexes()
        ...
        selected.push(
          $.map(columns, function (column) {
            ...
            if (editField !== undefined) {
              data = dtSettings.oApi._fnGetObjectDataFn(editField)(dt.row(cellIndex.row)
                .data())
            }
    ...
    _update: function (result, cells) {
      ...
      var dt = this.s.dt
      var cell
      var columns = dt.columns(this.c.columns)
        .indexes()
...
// events so we can automatically initialise
  $(document)
    .on('preInit.dt.autofill', function (e, settings, json) {
      if (e.namespace !== 'dt') {
        return
      }
