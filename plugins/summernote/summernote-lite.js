...
(function (global, factory) {
  ...
  var TooltipUI = /** @class */ (function () {
    ...
    TooltipUI.prototype.show = function () {
      ...
      $tooltip.addClass('in')
      $tooltip.find('.note-tooltip-content')
        .text(title)
  ...
  var DropdownUI = /** @class */ (function () {
    ...
    DropdownUI.prototype.show = function () {
      ...
      var windowWidth = $(window)
        .width()
      var targetMarginRight = parseFloat($(this.options.target)
        .css('margin-right'))
    ...
    DropdownUI.prototype.toggle = function () {
      var isOpened = this.$button.parent()
        .hasClass('open')
      this.clear()
      if (isOpened) {
        this.hide()
  ...
  var ModalUI = /** @class */ (function () {
    ...
    ModalUI.prototype.show = function () {
      ...
      this.$backdrop.appendTo(this.options.target)
        .show()
  ...
  var dropdownCheck = renderer.create('<div class="note-dropdown-menu note-check" role="list">', function ($node, options) {
    var markup = Array.isArray(options.items) ? options.items.map(function (item) {
      ...
      $temp.html([icon(options.checkClassName), ' ', content])
        .data('item', item)
      return $temp
    }) : options.items
    $node.html(markup)
  ...
  var palette = renderer.create('<div class="note-color-palette"/>', function ($node, options) {
    ...
    $node.find('.note-color-btn')
      .each(function () {
        $(this)
          .data('_lite_tooltip', new TooltipUI($(this), {
            container: options.container
          }))
      })
  })
      ...
      children: [
        ...
        dropdown({
          ...
          callback: function ($dropdown) {
            ...
            if (type === 'fore') {
              $dropdown.find('.btn-background-color')
                .hide()
              $dropdown.css({ 'min-width': '210px' })
            } else if (type === 'back') {
              $dropdown.find('.btn-foreground-color')
                .hide()
              $dropdown.css({ 'min-width': '210px' })
            }
          },
          click: function (event) {
            ...
            if (eventName && value) {
              ...
              var $currentButton = $button.closest('.note-color')
  ...
  var Context = /** @class */ (function () {
    ...
    Context.prototype.createInvokeHandler = function (namespace, value) {
      ...
      return function (event) {
        ...
        _this.invoke(namespace, value || $target.closest('[data-value]')
          .data('value'), $target)
      }
    }
  ...
  var WrappedRange = /** @class */ (function () {
    ...
    WrappedRange.prototype.pasteHTML = function (markup) {
      var contentsContainer = $$1('<div></div>')
        .html(markup)[0]
  ...
  var History = /** @class */ (function () {
    ...
    History.prototype.applySnapshot = function (snapshot) {
      ...
      if (snapshot.bookmark !== null) {
        range.createFromBookmark(this.editable, snapshot.bookmark)
          .select()
      }
    }
    /**
  ...
  var Bullet = /** @class */ (function () {
    ...
    Bullet.prototype.toggleList = function (listName, editable) {
      ...
      range.createFromParaBookmark(bookmark, paras)
        .select()
    }
    /**
     * @param {Node[]} paras
      ...
      $$1.each(clustereds, function (idx, paras) {
        ...
        if (headList.parentNode.nodeName === 'LI') {
          ...
          paras = isEscapseToBody ? dom.listDescendant(middleList, dom.isLi)
            : lists.from(middleList.childNodes)
              .filter(dom.isLi)
          // LI to P
  ...
  var Table = /** @class */ (function () {
    ...
    Table.prototype.addRow = function (rng, position) {
      var cell = dom.ancestor(rng.commonAncestor(), dom.isCell)
      var currentTr = $$1(cell)
        .closest('tr')
      var trAttributes = this.recoverAttributes(currentTr)
      var html = $$1('<tr' + trAttributes + '></tr>')
      var vTable = new TableResultAction(cell, TableResultAction.where.Row, TableResultAction.requestAction.Add, $$1(currentTr)
        .closest('table')[0])
        ...
        switch (currentCell.action) {
          ...
          case TableResultAction.resultAction.SumSpanCount:
            if (position === 'top') {
              ...
              if (isTopFromRowSpan) {
                var newTd = $$1('<div></div>')
                  .append($$1('<td' + tdAttributes + '>' + dom.blank + '</td>')
                    .removeAttr('rowspan'))
                  .html()
                html.append(newTd)
                break
    ...
    Table.prototype.addCol = function (rng, position) {
      ...
      var rowsGroup = $$1(row)
        .siblings()
      rowsGroup.push(row)
      var vTable = new TableResultAction(cell, TableResultAction.where.Column, TableResultAction.requestAction.Add, $$1(row)
        .closest('table')[0])
      var actions = vTable.getActionList()
    ...
    Table.prototype.deleteRow = function (rng) {
      ...
      var row = $$1(cell)
        .closest('tr')
      var cellPos = row.children('td, th')
        .index($$1(cell))
      var rowPos = row[0].rowIndex
      var vTable = new TableResultAction(cell, TableResultAction.where.Row, TableResultAction.requestAction.Delete, $$1(row)
        .closest('table')[0])
      var actions = vTable.getActionList()
      for (var actionIndex = 0; actionIndex < actions.length; actionIndex++) {
    ...
    Table.prototype.deleteCol = function (rng) {
      ...
      var cellPos = row.children('td, th')
        .index($$1(cell))
      var vTable = new TableResultAction(cell, TableResultAction.where.Column, TableResultAction.requestAction.Delete, $$1(row)
        .closest('table')[0])
      var actions = vTable.getActionList()
      for (var actionIndex = 0; actionIndex < actions.length; actionIndex++) {
        if (!actions[actionIndex]) {
  ...
  var Editor = /** @class */ (function () {
    function Editor(context) {
      ...
      this.insertNode = this.wrapCommand(function (node) {
        if (_this.isLimited($$1(node)
          .text().length)) {
          return
        }
        var rng = _this.getLastRange()
        rng.insertNode(node)
        range.createFromNodeAfter(node)
          .select()
        _this.setLastRange()
      })
      /**
      ...
      this.insertText = this.wrapCommand(function (text) {
        ...
        range.create(textNode, dom.nodeLength(textNode))
          .select()
        _this.setLastRange()
      })
      /**
      ...
      this.pasteHTML = this.wrapCommand(function (markup) {
        ...
        var contents = _this.getLastRange()
          .pasteHTML(markup)
        range.createFromNodeAfter(lists.last(contents))
          .select()
        _this.setLastRange()
      })
      /**
      ...
      this.insertHorizontalRule = this.wrapCommand(function () {
        var hrNode = _this.getLastRange()
          .insertNode(dom.create('HR'))
        if (hrNode.nextSibling) {
          range.create(hrNode.nextSibling, 0)
            .normalize()
            .select()
          _this.setLastRange()
        }
      ...
      this.createLink = this.wrapCommand(function (linkInfo) {
        ...
        range.create(startPoint.node, startPoint.offset, endPoint.node, endPoint.offset)
          .select()
        _this.setLastRange()
      })
      /**
    ...
    Editor.prototype.insertImage = function (src, param) {
      ...
      return createImage(src, param)
        .then(function ($image) {
          ...
          $image.show()
          range.create(_this.editable)
            .insertNode($image[0])
          range.createFromNodeAfter($image[0])
            .select()
          _this.setLastRange()
          _this.afterCommand()
        })
      ...
      if ($target && $target.length) {
        ...
        if ($target && $target.length) {
          ...
          if (className) {
            ...
            var $parent = $$1([currentRange.sc, currentRange.ec])
              .closest(tagName)
      ...
      if (rng) {
        ...
        if (rng.isCollapsed()) {
          ...
          if (firstSpan && !dom.nodeLength(firstSpan)) {
            ...
            range.createFromNodeAfter(firstSpan.firstChild)
              .select()
            this.setLastRange()
  ...
  var Dropzone = /** @class */ (function () {
    ...
    Dropzone.prototype.destroy = function () {
      var _this = this
      Object.keys(this.documentEventHandlers)
        .forEach(function (key) {
          _this.$eventListener.off(key.substr(2)
            .toLowerCase(), _this.documentEventHandlers[key])
        })
      this.documentEventHandlers = {}
    }
    return Dropzone
  ...
  var Statusbar = /** @class */ (function () {
    ...
    Statusbar.prototype.initialize = function () {
      ...
      this.$statusbar.on('mousedown', function (event) {
        ...
        _this.$document.on('mousemove', onMouseMove)
    ...
    Handle.prototype.initialize = function () {
      ...
      this.$handle.on('mousedown', function (event) {
        if (dom.isControlSizing(event.target)) {
          event.preventDefault()
          event.stopPropagation()
          var $target_1 = _this.$handle.find('.note-control-selection')
            .data('target')
  ...
  var Buttons = /** @class */ (function () {
        ...
        children: [
          ...
          this.ui.dropdown({
            ...
            callback: function ($dropdown) {
              ...
              $dropdown.find('.note-holder-custom')
                .each(function (idx, item) {
                  var $holder = $$1(item)
                  $holder.append(_this.ui.palette({
                    colors: customColors,
              ...
              $dropdown.find('input[type=color]')
                .each(function (idx, item) {
                  $$1(item)
                    .change(function () {
                      var $chip = $dropdown.find('#' + $$1(this)
                        .data('event'))
                        .find('.note-color-btn')
                        .first()
            ...
            click: function (event) {
              ...
              if (eventName === 'openPalette') {
                ...
                var $palette = $$1($parent.find('#' + $picker.data('event'))
                  .find('.note-color-row')[0])
                // Shift palette chips
                var $chip = $palette.find('.note-color-btn')
                  .last()
                  .detach()
                // Set chip attributes
    ...
    Buttons.prototype.updateCurrentStyle = function ($container) {
      ...
      if (styleInfo['font-family']) {
        var fontNames = styleInfo['font-family'].split(',')
          .map(function (name) {
        ...
        $cont.find('.note-current-fontname')
          .text(fontName_1)
          .css('font-family', fontName_1)
      }
      if (styleInfo['font-size']) {
        var fontSize_1 = styleInfo['font-size']
        $cont.find('.dropdown-fontsize a')
          .each(function (idx, item) {
            var $item = $$1(item)
        ...
        $cont.find('.note-current-fontsize')
          .text(fontSize_1)
      }
      if (styleInfo['line-height']) {
        var lineHeight_1 = styleInfo['line-height']
        $cont.find('.dropdown-line-height li a')
          .each(function (idx, item) {
            // always compare with string to avoid creating another func.
            var isChecked = ($$1(item)
              .data('value') + '') === (lineHeight_1 + '')
  ...
  var Toolbar = /** @class */ (function () {
    ...
    Toolbar.prototype.followScroll = function () {
      ...
      if (this.options.otherStaticBar) {
        otherBarHeight = $$1(this.options.otherStaticBar)
          .outerHeight()
      }
  ...
  var HelpDialog = /** @class */ (function () {
    ...
    HelpDialog.prototype.initialize = function () {
      ...
      this.$dialog = this.ui.dialog({
        ...
        footer: body,
        callback: function ($node) {
          $node.find('.modal-body,.note-modal-body')
            .css({
              'max-height': 300,
              'overflow': 'scroll'
            })
    ...
    HelpDialog.prototype.createShortcutList = function () {
      ...
      return Object.keys(keyMap)
        .map(function (key) {
          ...
          $row.append($$1('<label><kbd>' + key + '</kdb></label>')
            .css({
              'width': 180,
              'margin-right': 10
            }))
  ...
  var HintPopover = /** @class */ (function () {
    ...
    HintPopover.prototype.moveDown = function () {
      ...
      if ($next.length) {
        ...
        var $nextGroup = $current.parent()
          .next()
        if (!$nextGroup.length) {
          $nextGroup = this.$content.find('.note-hint-group')
            .first()
        }
        this.selectItem($nextGroup.find('.note-hint-item')
          .first())
      }
    }
    HintPopover.prototype.moveUp = function () {
      ...
      if ($prev.length) {
        ...
        var $prevGroup = $current.parent()
          .prev()
        if (!$prevGroup.length) {
          $prevGroup = this.$content.find('.note-hint-group')
            .last()
        }
        this.selectItem($prevGroup.find('.note-hint-item')
          .last())
      }
    }
    ...
    HintPopover.prototype.handleKeyup = function (e) {
      ...
      if (!lists.contains([key.code.ENTER, key.code.UP, key.code.DOWN], e.keyCode)) {
        var wordRange = this.context.invoke('editor.getLastRange')
          .getWordRange()
          ...
          if (bnd) {
            ...
            this.hints.forEach(function (hint, idx) {
              if (hint.match.test(keyword_1)) {
                _this.createGroup(idx, keyword_1)
                  .appendTo(_this.$content)
              }
            })
            // select first .note-hint-item
            this.$content.find('.note-hint-item:first')
              .addClass('active')
