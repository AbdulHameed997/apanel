...
(function (factory) {
  ...
  $.extend(KeyTable.prototype, {
    ...
    _constructor: function () {
      ...
      $(dt.table()
        .body())
        .on('click' + namespace, 'th, td', function (e) {
      ...
      if (this.c.blurable) {
        $(document)
          .on('mousedown' + namespace, function (e) {
            ...
            if ($(e.target)
              .parents('.dataTables_filter').length) {
              that._blur()
            }
            ...
            if ($(e.target)
              .parents('div.editor-datetime').length) {
              return
            }
      ...
      if (this.c.editor) {
        ...
        $(dt.table()
          .body())
          .on('dblclick' + namespace, 'th, td', function (e) {
            if (that.s.enable === false) {
              return
            }
      ...
      dt.on('draw' + namespace, function (e) {
        ...
        if (lastFocus && lastFocus.node && $(lastFocus.node)
          .closest('body') === document.body) {
    ...
    _clipboard: function () {
      ...
      $(document)
        .on('copy' + namespace, function (ejq) {
      ...
      $(document)
        .on('paste' + namespace, function (ejq) {
          var e = ejq.originalEvent
          var focused = that.s.lastFocus
    ...
    _columns: function () {
      var dt = this.s.dt
      var user = dt.columns(this.c.columns)
        .indexes()
      var out = []
    ...
    _editor: function (key, orig, hardEdit) {
      ...
      if (key === 13) {
        ...
        $(document)
          .one('keyup', function () { // immediately removed
            editInline()
          })
      } else {
        editInline()
    ...
    _focus: function (row, column, shift, originalEvent) {
      ...
      if (shift === undefined || shift === true) {
        ...
        var bodyParent = dt.table()
          .body().parentNode
        if (bodyParent !== dt.table()
          .header().parentNode) {
    ...
    _key: function (e) {
      ...
      if (!this.s.dt.cell(lastFocus.node)
        .any()) {
        this.s.lastFocus = null
        return
      }
      ...
      switch (e.keyCode) {
        ...
        case 36: // home (start of current page)
          if (navEnable) {
            e.preventDefault()
            var indexes = dt.cells({ page: 'current' })
              .indexes()
    ...
    _scroll: function (container, scroller, cell, posOff) {
      ...
      if (posOff === 'position') {
        offset.top += parseInt(cell.closest('table')
          .css('top'), 10)
      }
...
// events so we can automatically initialise
  $(document)
    .on('preInit.dt.dtk', function (e, settings, json) {
      if (e.namespace !== 'dt') {
        return
      }
