...
function firstWeekOffset(year, dow, doy) {
  ...
  var fwdlw = (7 + arrayToUtcDate([year, 0, fwd])
    .getUTCDay() - dow) % 7
  return -fwdlw + fwd - 1
}
...
function queryRawLocale(codes, available) {
  for (var i = 0; i < codes.length; i++) {
    var parts = codes[i].toLocaleLowerCase()
      .split('-')
    for (var j = parts.length; j > 0; j--) {
      var simpleId = parts.slice(0, j)
        .join('-')
  ...
  DateEnv.prototype.offsetForMarker = function (m) {
    if (this.timeZone === 'local') {
      return -arrayToLocalDate(dateToUtcArray(m))
        .getTimezoneOffset() // convert "inverse" offset to "normal" offset
...
var Toolbar = /** @class */ (function (_super) {
  ...
  Toolbar.prototype.renderSection = function (position, buttonStr) {
    ...
    if (buttonStr) {
      buttonStr.split(' ')
        .forEach(function (buttonGroupStr, i) {
          var groupChildren = []
          var isOnlyButtons = true
          var groupEl
          buttonGroupStr.split(',')
            .forEach(function (buttonName, j) {
  ...
  Toolbar.prototype.toggleButtonEnabled = function (buttonName, bool) {
    findElements(this.el, '.fc-' + buttonName + '-button')
      .forEach(function (buttonEl) {
  ...
  View.prototype.startNowIndicator = function (dateProfile) {
    ...
    if (this.opt('nowIndicator')) {
      ...
      if (unit) {
        ...
        delay = dateEnv.add(dateEnv.startOf(this.initialNowDate, unit), createDuration(1, unit))
