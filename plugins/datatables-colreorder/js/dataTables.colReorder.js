...
(function (factory) {
  ...
  $.fn.dataTableExt.oApi.fnColReorder = function (oSettings, iFrom, iTo, drop, invalidateRows) {
    ...
    $(oSettings.oInstance)
      .trigger('column-reorder.dt', [oSettings, {
        from: iFrom,
        to: iTo,
  ...
  $.extend(ColReorder.prototype, {
    ...
    '_fnConstruct': function () {
      ...
      if (aiOrder) {
        ...
        if (!that.s.dt._bInitComplete) {
          var bDone = false
          $(table)
            .on('draw.dt.colReorder', function () {
      ...
      // Destroy clean up
      $(table)
        .on('destroy.dt.colReorder', function () {
          $(table)
            .off('destroy.dt.colReorder draw.dt.colReorder')

          $.each(that.s.dt.aoColumns, function (i, column) {
            $(column.nTh)
              .off('.ColReorder')
            $(column.nTh)
              .removeAttr('data-column-index')
          })
    ...
    '_fnMouseDown': function (e, nTh) {
      ...
      var idx = parseInt($(nTh)
        .attr('data-column-index'), 10)
    ...
    '_fnRegions': function () {
      ...
      var lastBound = $(this.s.dt.nTable)
        .offset().left
      ...
      var firstColumnWidth = $(aoColumns[firstColumn.index].nTh)
        .outerWidth()
...
// events so we can automatically initialise
  $(document)
    .on('preInit.dt.colReorder', function (e, settings) {
      if (e.namespace !== 'dt') {
        return
      }
